...
* *R interoperability*: The R *spatialdataR* package gained support for Zarr v2/v3 read/write, spatial, table-based queries and coordinate transformations. Several new and extended R packages improve OME-Zarr handling (*ImageArray* package), cross-language AnnData[@Virshup2024-cn] compatibility (Zarr support for *anndataR* package), and conformance with Open Microscopy Environment Next-Generation File Format (OME-NGFF) specifications (*romeo* package).
